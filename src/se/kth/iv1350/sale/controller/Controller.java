...
public class Controller {
	 ...
	 */
	public PurchaseDTO registerItem(int itemID, int quantity) {
		boolean isValid = inventorySys.getItemValidity(itemID);
		ItemDTO item = inventorySys.getItem(itemID);
		currentPurchase.addItem(item, quantity, isValid);
	 ...
	 */
	public ReceiptDTO processPayment(Amount amountPaid) {
		PurchaseDTO saleInfo = currentPurchase.getPurchaseDTO();
		Payment payment = new Payment(saleInfo, amountPaid);
		payment.logSale();
		updateExternalSystems(saleInfo);	
		ReceiptDTO receipt = printReceipt(payment);
		return receipt;
	}
	...
	private ReceiptDTO printReceipt(Payment payment) {
